...
repo_root <- resolve_repo_root(file.path("scripts", "prepare_isoform_normalization.R"))
setwd(repo_root)
...
load_and_filter_isoform_expression <- function(cohort_label, genes_config = "config/genes.csv") {
  ...
  if (tolower(cohort_label) == "pancan") {
    ...
    sample_ids_canonical <- canonicalize_matrix_sample_ids(sample_ids_raw)
    tcga_primary_tumor_keep <- grepl("^TCGA-[^-]+-[^-]+-01$", sample_ids_canonical, perl = TRUE)
    if (!any(tcga_primary_tumor_keep)) {
      stop(
        "Pancan isoform-expression table does not contain any canonicalized TCGA primary-tumor sample IDs with sample type code 01.",
        call. = FALSE
      )
    }
    dropped_count <- sum(!tcga_primary_tumor_keep)
    if (dropped_count > 0L) {
      message(
        "Dropping ", dropped_count,
        " non-TCGA or non-primary-tumor pancan columns before normalization."
      )
    }
    isoform_expr_df <- isoform_expr_df[, c(TRUE, tcga_primary_tumor_keep), drop = FALSE]
  }
